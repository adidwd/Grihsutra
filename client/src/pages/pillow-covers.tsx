...
export default function PillowCovers() {
  const [sortBy, setSortBy] = useState("name");
  const [filterBy, setFilterBy] = useState("all");

  const { data: products = [], isLoading } = useQuery<Product[]>({
    queryKey: ["/api/products/category/pillow-covers"],
  });
      ...
      <section className="bg-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">Pillow Covers Collection</h1>
            <p className="text-xl text-neutral">Stylish comfort for your perfect sanctuary</p>
          </div>
        </div>
      </section>
